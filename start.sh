#!/usr/bin/env bash
# CAAMS Enterprise startup script — serves over HTTPS using certs/cert.pem + certs/key.pem
#
# First-time setup:
#   mkdir -p certs
...
if [[ -z "${CAAMS_SECRET_KEY:-}" ]]; then
  echo "ERROR: CAAMS_SECRET_KEY is not set — the application will not start."
  echo "Generate and export a key with:"
